...
(* 
  an optimized version could be realized by creating a dictionary from lit to
  list of clause index; but since our simpl si desctructive, we cannot;
*)
...
(* simplify a cnf; if c' is [Const false], we should stop from propagating modifications *)
  let rec simpl_fail b = 
  List.fold_left (fun acc c -> 
    let c' = clause_solve c 0 0 [] in 
    match c' with 
    | [Const true] -> acc
    | [Const false] -> failwith "false clause"
    | _ -> c'::acc
  ) [] b
;;
